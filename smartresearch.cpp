...
SmartResearch::SmartResearch(QWidget *parent)
  ...
  // Try to find if ui->pays is in a layout to insert it next to it
  if (ui->pays->parentWidget() && ui->pays->parentWidget()->layout()) {
       ui->pays->parentWidget()->layout()->addWidget(btnMap);
  } else {
       // Fallback: move it manually if layout is not detected (Qt Designer layouts can be tricky)
...
void SmartResearch::updateJournalStats() {
    ...
    // --- Collect per-country counts for each type ---
    QMap<QString, int> confData;
    QMap<QString, int> jourData;

    for (auto it = stats.begin(); it != stats.end(); ++it) {
        if (it.key().startsWith("pays_Conf") && it.value() > 0) {
            int second_us = it.key().indexOf('_', 5); // find underscore after "pays_"
            if (second_us != -1) {
    ...
    // Fallback: if no country data, show type totals
    ...
    // --- Helper: build a styled pie chart ---
    auto makePieChart = [&](const QMap<QString, int>& data, const QString& title) -> QChartView* {
        ...
        for (auto it = data.begin(); it != data.end(); ++it) {
            ...
            slice->setBorderColor(QColor(15, 23, 42)); // dark border between slices
        ...
        // Hide chaotic outside annotations to prevent overlapping strings
        series->setLabelsVisible(false);
        series->setPieSize(0.80); // Taille du camembert optimale
        ...
        // Destruction absolue de TOUTES les marges QtCharts qui écrasent le camembert
        ...
        // Déplacement de la légende sur la DROITE : liste verticale parfaite car le texte est large !
        ...
        // FORCE une taille minimale pour interdire à Qt d'écraser le widget à 10 pixels !!
    ...
    // --- Embed side by side in groupBox_Stats_Journal ---
    ui->label_Chart_Journal->hide();

    // Nettoyage robuste du layout et des sous-widgets pour éviter les superpositions graphiques
    // Utilisation de deleteLater() et exclusion stricte du label Designer original pour éviter le crash !
    ...
    // === Interactive Publication Counter UI ===
    ...
    // Dynamic SQL Counting setup using lambda
    ...
    mainLayout->addLayout(chartsLayout, 1); // stretch
    mainLayout->addWidget(interactiveContainer, 0); // no vertical stretch
...
void SmartResearch::on_enregistrer1_clicked()
{
    ...
    if (j.ajouter()) {
        ...
        // Ouverture directe du site web s'il a été fourni
        if (!site.isEmpty()) {
            QUrl webUrl(site);
...
void SmartResearch::fetchJournalAPI()
{
    ...
    // Sauvegarde du style original avant modification
    ...
    // URL OpenAlex pour rechercher la source par son nom (1 seul résultat max)
...
void SmartResearch::onJournalAPIResponse(QNetworkReply *reply)
{
    if(reply->error() == QNetworkReply::NoError) {
        ...
        if(!results.isEmpty()) {
            ...
            // Decodage manuel des codes pays principaux pour assurer le bon matching
            ...
            if(impact > 0) {
                ...
                // Estimation très approximative du Quartile selon le volume de citations moyen
                if(impact >= 4.0) ui->classement->setCurrentIndex(1); // Q1
                else if(impact >= 2.0) ui->classement->setCurrentIndex(2); // Q2
                else if(impact >= 1.0) ui->classement->setCurrentIndex(3); // Q3
                else ui->classement->setCurrentIndex(4); // Q4
            }
            ...
            if(!country.isEmpty()) {
                ...
                if(idx != -1) {
                    ...
                    // Force inject if not found verbatim
            ...
            QMessageBox::information(this, "API OpenAlex : Succès", "Le journal a été trouvé mondialement !\nUn maximum de boîtes a été auto-rempli.");
             ...
             // Restauration du style original (Garde le fond blanc)
