...
bool Reviewer::ajouterReviewer()
{
    ...
    QSqlQuery query;
    query.prepare("INSERT INTO TABLE_REVIEWER (ID_REVIEWER, NOM_REVIEWER, EMAIL_REVIEWER, \"SPÉCIALITÉ_REVIEWER\", "
...
QSqlQueryModel* Reviewer::afficher()
{
    ...
    model->setQuery("SELECT ID_REVIEWER, NOM_REVIEWER, EMAIL_REVIEWER, USER_NAME_REVIEWER, TELEPHONE_REVIEWER, PHOTO_REVIEWER, \"SPÉCIALITÉ_REVIEWER\", INSTITUTION_REVIEWER, STATUT_REVIEWER FROM TABLE_REVIEWER ORDER BY ID_REVIEWER ASC");
...
bool Reviewer::modifier(int originalId)
{
    ...
    query.prepare("UPDATE TABLE_REVIEWER SET "
                  ...
                  "\"SPÉCIALITÉ_REVIEWER\" = :specialite, "
...
QMap<QString, int> Reviewer::getStatistics()
{
    ...
    // Per specialite
    query.prepare("SELECT \"SPÉCIALITÉ_REVIEWER\", COUNT(*) FROM TABLE_REVIEWER GROUP BY \"SPÉCIALITÉ_REVIEWER\"");
    if (query.exec()) {
        while (query.next()) {
