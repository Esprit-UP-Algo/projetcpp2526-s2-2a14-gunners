...
// SPÉCIALITÉ_REVIEWER — É = U+00C9 = UTF-8 0xC3 0x89
// Encoded explicitly to avoid source-file encoding corruption.
static const QString SPEC_COL = QString::fromUtf8("\"SP\xc3\x89CIALIT\xc3\x89_REVIEWER\"");
...
bool Reviewer::ajouterReviewer()
{
    ...
    QSqlQuery query;
    query.prepare("INSERT INTO TABLE_REVIEWER (ID_REVIEWER, NOM_REVIEWER, EMAIL_REVIEWER, " + SPEC_COL + ", "
...
QSqlQueryModel* Reviewer::afficher()
{
    ...
    model->setQuery("SELECT ID_REVIEWER, NOM_REVIEWER, EMAIL_REVIEWER, USER_NAME_REVIEWER, TELEPHONE_REVIEWER, PHOTO_REVIEWER, " + SPEC_COL + ", INSTITUTION_REVIEWER, STATUT_REVIEWER FROM TABLE_REVIEWER ORDER BY ID_REVIEWER ASC");
...
QMap<QString, int> Reviewer::getStatistics()
{
    ...
    // Per specialite
    query.prepare("SELECT " + SPEC_COL + ", COUNT(*) FROM TABLE_REVIEWER GROUP BY " + SPEC_COL);
    if (query.exec()) {
        while (query.next()) {
