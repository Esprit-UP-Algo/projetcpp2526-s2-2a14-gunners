...
bool Publication::controler_saisie()
{
    if (id_pub <= 0) {
    ...
    if (titre_pub.isEmpty()) {
        QMessageBox::warning(nullptr, "Erreur de saisie", "Le champ 'Titre' est obligatoire.");
        return false;
    }
    if (auteurs_pub.isEmpty()) {
        QMessageBox::warning(nullptr, "Erreur de saisie", "Le champ 'Auteurs' est obligatoire.");
        return false;
    }
    if (abstract_pub.isEmpty()) {
        QMessageBox::warning(nullptr, "Erreur de saisie", "Le champ 'Abstract' est obligatoire.");
        return false;
    }
    if (type_pub.isEmpty()) {
        QMessageBox::warning(nullptr, "Erreur de saisie", "Veuillez sélectionner le 'Type' (Conférence ou Journal).");
        return false;
    }
    if (mots_cles_pub.isEmpty()) {
        QMessageBox::warning(nullptr, "Erreur de saisie", "Le champ 'Mots clés' est obligatoire.");
        return false;
    }
    if (statut_pub.isEmpty()) {
        QMessageBox::warning(nullptr, "Erreur de saisie", "Le champ 'Statut' est obligatoire.");
        return false;
    }
    return true;
}
